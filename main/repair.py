...
for idir in range(mainconfig["nthreads"]):
    ...
    shutil.copytree(mainconfig["synthVpath"], f'SynthV_core{idir}',dirs_exist_ok=True)

    #shutil.rmtree(f'convolve_core{idir}', ignore_errors=True)
    #shutil.copytree(mainconfig["convolvepath"], f'convolve_core{idir},dirs_exist_ok=True')
...
lcf = np.loadtxt(mainconfig["path_lc"],skiprows=1)
if lcf[0][0] < 2457000.0:
    lcf[:,0] += 2457000.0
try:
    if mainconfig["obsmode"] == "test":
